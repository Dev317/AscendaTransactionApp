...
EXCLUSION_INDEX_TABLE_NAME = os.environ.get(
    "EXCLUSION_INDEX_TABLE_NAME", "exclusion_index_table"
)
...
def create_exclusion(data):
    ...
    for exclusion in exclusion_index_list:
        if exclusion_id == exclusion:
            ...
            raise DuplicateExclusionIndex(
                "Duplicate exclusion id detected, aborting exclusion creation"
            )
...
def get_index():
    ...
    try:
        index_response = EXCLUSION_INDEX_TABLE.get_item(
            Key={"exclusion_index_id": "exclusion_index"}
        )
...
def get_all():
    ...
    try:
        response = DYNAMODB_CLIENT.batch_get_item(
            RequestItems={
                EXCLUSION_TABLE_NAME: {"Keys": keys_list, "ConsistentRead": True}
            },
