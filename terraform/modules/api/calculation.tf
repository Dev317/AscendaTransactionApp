...
resource "aws_api_gateway_integration" "create_calculation_lambda" {
  rest_api_id = aws_api_gateway_rest_api.orchestrator_apigw.id
  resource_id = aws_api_gateway_method.post_calculation.resource_id
...
resource "aws_lambda_permission" "apigw-create-calculation" {
...
resource "aws_api_gateway_deployment" "calculation_api_deployment" {
  depends_on = [ aws_api_gateway_integration.create_calculation_lambda ]
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_integration.create_calculation_lambda.id,
    ]))
  }
...
resource "aws_api_gateway_stage" "calculation_prod_stage" {
  deployment_id = aws_api_gateway_deployment.calculation_api_deployment.id
  rest_api_id   = aws_api_gateway_rest_api.orchestrator_apigw.id
  stage_name    = "prod"
}
